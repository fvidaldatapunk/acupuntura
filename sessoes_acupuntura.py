...
#caminho = 'sessoes_acupuntura_completo.xlsx'
...
df_sessoes['dia'] = pd.to_datetime(df_sessoes['dia'])
df_sessoes['Dia da semana'] = df_sessoes['dia'].dt.weekday.map(dias)
df_sessoes['Ano'] = df_sessoes['dia'].dt.year
df_sessoes['Mês'] = df_sessoes['dia'].dt.month_name()


#VISÃO ANUAL
soma_ano = df_sessoes['Valor (R$)'].sum()

df_sessoes['Valor perc'] = (df_sessoes['Valor (R$)']/soma_ano   )*100

df_perc_anual = df_sessoes.groupby('Ano')['Valor perc'].sum().reset_index()

df_sessoes_anual = (df_sessoes.groupby('Ano').agg(
...
graf_anual.update_traces(texttemplate='%{y:.2f}%',textposition='outside')

#VISÃo MENSAL
ordem_meses = ['January', 'February', 'March', 'April', 'May', 'June',
               'July', 'August', 'September', 'October', 'November', 'December']

df_sessoes_mensal = (df_sessoes.groupby(['Ano','Mês']).agg(
    qt_sessoes = ('Nome do Paciente','count'),
    numero_pacientes = ('Nome do Paciente','nunique'),
...
graf_mes_atual = px.bar(df_sessoes_mes,x='Mês',y='Nome do Paciente', title='Ano Vigente por mês', text='Nome do Paciente')
graf_mes_atual.update_traces(textposition = 'outside')
graf_mes_atual.update_layout(xaxis_title='Mês', yaxis_title='Qtd Pacientes')


#tipo consulta consolidado
df_tipo_consulta = (df_sessoes.groupby('Tipo de Consulta').agg(
...
df_tipo_consulta_anual = (df_sessoes.groupby(['Ano','Tipo de Consulta']).agg(
    qt_tipo_consulta = ('Tipo de Consulta','count'),
    valor_tipo_consulta = ('Valor (R$)','sum'),
    turno_relevante = ('Turno',lambda x: x.mode()[0])
)).reset_index()
...
df_terapeuta_cons = (df_sessoes.groupby('Terapeuta').agg(
    qt_sessoes = ('Nome do Paciente','count'),
...
#PACIENTE
df_paciente =(df_sessoes.pivot_table(
...
df_faixa_anual = df_sessoes.groupby(['Ano','Faixa etaria'])['Nome do Paciente'].count().reset_index()
...
graf_etaria.update_layout(xaxis_title='Ano', yaxis_title='Qtd Pacientes')

#MOTIVO

df_motivo= (df_sessoes.groupby(['Motivo','Faixa etaria']).agg(
    qt_sessoes = ('Nome do Paciente','count')
    )).reset_index()

df_motivo_ano= df_sessoes.groupby(['Ano','Motivo'])['Nome do Paciente'].count().reset_index()
...
graf_motivo.update_layout(xaxis_title='Ano', yaxis_title='Qtd Pacientes')
